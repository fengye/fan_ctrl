...
try:
    while (1):
        ...
        if(abs(cpuTemp-cpuTempOld > hyst)):
            ...
            # Below threshold value, fan will stop 
            if(cpuTemp < stopFanTemp):
               fanSpeed = 0

            # Between threshold value and first step temp, use the lowest fan speed
            elif(cpuTemp >= stopFanTemp and cpuTemp <= tempSteps[0]):
                   # However gives more time for fan to rest if it was in stop state
                   if fanSpeedOld != 0:
